...
districts = gpd.read_file('files/bgd_adm_bbs_20201113_SHP/bgd_admbnda_adm2_bbs_20201113.shp')
upazilas = gpd.read_file('files/bgd_adm_bbs_20201113_SHP/bgd_admbnda_adm3_bbs_20201113.shp')
unions = gpd.read_file('files/bgd_adm_bbs_20201113_SHP/bgd_admbnda_adm4_bbs_20201113.shp')
...
def visualize_intersecting_unions(intersecting_unions, intersecting_upazilas, intersecting_districts, tiff_gdf, max_union_name, max_upazila_name, tif_img, centroid):
    with open('files/metadata_backbone.json', 'r') as f:
...
for folder in folder_list:
    ...
    print(f"Processing {len(tif_files)} TIFF files in folder: {folder}")        ##

    for tif_img in tqdm(tif_files, desc=f"Processing {os.path.basename(folder)}"): ##
